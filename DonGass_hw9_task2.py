...
def monthly_payment(loan_amount, yearly_interest_rate, 
    ...
    cumulative_payment = [(i * payment) + initial_point_payment for i, x in enumerate(months)]
...
# Calculate and plot for 30 yr fixed at 5% APR after paying $6500 point payment up front
months, cumulative_payment = monthly_payment(200000, 0.05, 360, 6500)
plt.plot(months, cumulative_payment, label="30 yr fixed, 5% APR, $6500 pt payment")

# Setup plot labels, legends, etc.
plt.xlabel("Months (0 - 360)")
plt.ylabel("Cumulative payments made")
plt.title("30 yr fixed vs points")
